...
.player-frame { position: relative; aspect-ratio: 16 / 9; background: #000; border-radius: 10px; overflow: hidden; border: 1px solid var(--border); }
.player-frame iframe { position: absolute; inset: 0; width: 100%; height: 100%; border: 0; }
.chapter-pane {
  background: var(--surface); border: 1px solid var(--border); border-radius: 10px;
  padding: .9rem; min-height: 0; display: flex; flex-direction: column;
}
.pane-title { margin: 0 0 .5rem; font-size: .78rem; text-transform: uppercase; letter-spacing: .08em; color: var(--muted); }
.chapters { list-style: none; margin: 0; padding: 0; overflow-y: auto; flex: 1; max-height: 480px; }
